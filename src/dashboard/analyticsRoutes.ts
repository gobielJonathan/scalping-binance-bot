...
router.get("/summary", async (req, res) => {
  try {
    const { startDate, endDate, symbols, mode = "paper" } = req.query;
...
router.get("/report", async (req, res) => {
  try {
    ...
    logger.error("Analytics report error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/performance/symbol", async (req, res) => {
  try {
    const { startDate, endDate, mode = "paper" } = req.query;
    ...
    logger.error("Symbol performance error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/performance/time-of-day", async (req, res) => {
  try {
    ...
    const performance =
      await analyticsService.getPerformanceByTimeOfDay(options);
    res.json(performance);
  } catch (error) {
    logger.error("Time of day performance error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/performance/day-of-week", async (req, res) => {
  try {
    ...
    const performance =
      await analyticsService.getPerformanceByDayOfWeek(options);
    res.json(performance);
  } catch (error) {
    logger.error("Day of week performance error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/drawdown", async (req, res) => {
  try {
    ...
    logger.error("Drawdown analysis error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/streaks", async (req, res) => {
  try {
    ...
    logger.error("Streak analysis error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/risk", async (req, res) => {
  try {
    ...
    logger.error("Risk metrics error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/trades", async (req, res) => {
  try {
    ...
    const countResult = (await databaseService.queryGet(
      countQuery,
      countParams,
    )) as { total: number };
    ...
    logger.error("Trade history error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const [usdt] = usdtBalances || [{ total: 0, free: 0 }];
    ...
    const profitFactor =
      result.avgLoss > 0
        ? (result.winningTrades * result.avgWin) /
          (result.losingTrades * result.avgLoss)
        : 0;
    ...
    logger.error("Analytics stats error:", {
      error:
        error instanceof Error
          ? { stack: error.stack, code: (error as any).code }
          : { stack: String(error) },
