...
export interface Config {
  ...
  trading: {
    mode: "paper" | "live";
    tradingAccount: "isolated_margin";
    leverage: number;
...
const config: Config = {
  ...
  trading: {
    ...
    tradingAccount: "isolated_margin" as const,
    leverage: Math.max(1, parseInt(process.env.LEVERAGE || "3")),
    marginMaintenanceRate: parseFloat(
      process.env.MARGIN_MAINTENANCE_RATE || "0.05",
    ),
    ...
    takeProfitPercentage: parseFloat(
      process.env.TAKE_PROFIT_PERCENTAGE || "0.006",
    ), // 0.6% for 1:2 risk-reward ratio
    ...
    pairSelectionMode: (() => {
      ...
      if (mode !== undefined) {
        console.warn(
          `Warning: PAIR_SELECTION_MODE="${mode}" is not valid — defaulting to "static"`,
        );
      }
      return "static";
    })(),
    ...
    volatilityRefreshInterval: parseInt(
      process.env.VOLATILITY_REFRESH_INTERVAL || "3600000",
    ),
  },
  indicators: {
